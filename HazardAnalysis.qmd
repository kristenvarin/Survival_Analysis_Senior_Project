...
# define functions
a = function(a, L, n, sum_ln_x) a - ((n*log(L) - n * digamma(a) + sum_ln_x) / (-n * trigamma(a)))
L = function(a, L, n, sum_x) L - (((n*a/L) - sum_x) / (-n * a / L^2))
...
while (TRUE) {
  a1 <- a_func(a, L, n, sum_ln_x)
  L1 <- L_func(a, L, n, sum_x)
...
results<- data.frame(iteration = 1:length(iterations), a = iterations[seq(1, length(iterations), 2)], L = iterations[seq(2, length(iterations), 2)])
