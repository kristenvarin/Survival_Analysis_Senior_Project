...
First, lets view the first 6 survival times.

```{r}
head(Surv(heart$TIME, heart$Event))
```

Let's run a cox proportional hazards model to see if blood pressure is a significant predictor of the hazard of death for heart failure patients. In this data set, blood pressure is a binary variable where 0 indicates normal blood pressure and 1 indicates high blood pressure. It is called BP in the data set.
